...
void HistSet::book(TFileDirectory subDir, const std::string& post) {
  ...
  timeVsEta_           =(TH2F*) subDir.make<TH2F>("timeVsEta","timeVsEta;#eta; time [ns]",100,-3.5,3.5,100,-20.0,20.0);
  ...
  timeVsPhi_           =(TH2F*) subDir.make<TH2F>("timeVsPhi","timeVsPhi; #phi; time [ns]",100,-3.5,3.5,100,-20.0,20.0);
  seedAmpli_           =(TH1F*) subDir.make<TH1F>("E(seed)  ","E(seed) ; E [GeV]",130,0,130);
  timeVsSeedE_         =(TH2F*) subDir.make<TH2F>("timeVsSeedE","timeVsSeedE;seed E[GeV]; time [ns]",150,0.0,150,100,-10.0,10.0);
  ...
  gammaEt1_        =(TH1F*) subDir.make<TH1F>("Et_{1}^{#gamma} ","Et_{1}^{#gamma}; E_t_{1}^{#gamma}",150,0.0,150.0);
  gammaEt2_        =(TH1F*) subDir.make<TH1F>("Et_{2}^{#gamma} ","Et_{2}^{#gamma}; E_t_{2}^{#gamma}",150,0.0,150.0);
  ...
  NZmass_         =(TH1F*) subDir.make<TH1F>("NZmass","NZmass; m(#gamma,#gamma) [GeV]",80,50,130);
...
void HistSet::fill(int sc1, int sc2, int bc1, int bc2 ){
  ...
  // if( fabs(diEle.M()  - 91) < 5 )
   ...
   //signal==>
  if( diEle.M() > 80.0 && diEle.M() < 100.0 ){ NZmass_->Fill(diEle.M()); NZmassseedTime_ ->Fill( bcTime1.seedtime) ; NZmassseedTime_->Fill( bcTime2.seedtime);} 
   // Background==>
  //if( diEle.M() < 80.0 ||  diEle.M() > 100.0 ){ NZmass_->Fill(diEle.M()); NZmassseedTime_ ->Fill( bcTime1.seedtime) ; NZmassseedTime_->Fill( bcTime2.seedtime);} 
...
if((diEle.M() > 78.0) && (diEle.M() < 96.0)){
    //Real Signal Positive Time
    if( (bcTime1.seedtime > 3.10 && bcTime1.seedtime < 10.1 )  ||  ( bcTime2.seedtime > 3.10  &&  bcTime2.seedtime < 10.1 )  ) { ZMassMoreSeedTime_->Fill(bcTime1.seedtime); ZMassMoreSeedTime_->Fill(bcTime2.seedtime);std::cout<<"Got Nobel!" << std::endl;}
 
    //Real Signal Negative Time
    if(( bcTime1.seedtime > -10.1 &&  bcTime1.seedtime < -3.10 )  ||  ( bcTime2.seedtime > -10.1 && bcTime2.seedtime < -3.10 ) ) { ZMassLessSeedTime_->Fill(bcTime1.seedtime); ZMassLessSeedTime_->Fill(bcTime2.seedtime);std::cout<<"Got Nobel!" << std::endl;}
