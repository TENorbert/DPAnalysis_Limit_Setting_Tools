...
#  cmsenv
# ./UseGridLimit.plx  <Data Card>  <Output root file>  < Output Log file name> 
#
# e.g
...
$cmd="combine -v0 -n DPp -m $Lambda -M ProfileLikelihood -t 500 $dfile|";
open(LOG,">${log}");
print LOG "===== $cmd \n";
open(SEED,$cmd);
while (<SEED>) {
...
$rv=exp($rv);
$seed=$i+248100;
$cmd="combine -v0 -n DPp -m $Lambda -M HybridNew --clsAcc 0 -t 300 --saveToys --saveHybridResult --singlePoint $rv -s $seed --frequentist --testStat LHC $dfile > ${i}.log 2>&1 ";
print LOG "===== $cmd \n";
